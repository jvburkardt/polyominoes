function [p_shapes, r_shape, p_num, d, m, n, f] = LPmake_polyominoes_Figure8 ()

%*****************************************************************************80
%
%% LPmake_polyominoes_Figure8 () creates the LP file for Figure 8 in the reference below.
%
%  Discussion:
%
%    33 copies of an L-shaped trimino and 9 copies of the cross-shaped
%    pentomino are used to tile a jagged-square shaped region with a single ‘hole’ 
%    in the middle.
%
%    This problem is depicted in figure 8 in the reference given below.
%    Note that this problem does not use colouring techniques, i.e., we
%    tile with polyominoes (not pariominoes). The actual figure 8 in the
...
%  Output:
%
%    integer P_SHAPES(13,13,2): a binary matrix that describes each polyomino.
%
...
%
  ...
  p_shapes(1:2,1:2,1) = [ 1, 0; 
                                          1, 1];
                                                                        
  p_shapes(1:3,1:3,2) = [0, 1, 0;
                                          1, 1, 1;
                                          0, 1, 0];
%
...
%
  filename ='Figure8_polyominoes.lp';
  label = '\ LP file for Figure8 in the pariomino paper (no colouring techniques).';
  polyomino_lp_write ( filename, label, m, n, a, b );
  
  fprintf ( 1, '\n' );
  fprintf ( 1, '  LPmake_polyominoes_Figure8 created the LP file "%s"\n', filename );
